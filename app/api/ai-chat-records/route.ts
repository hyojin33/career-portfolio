...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    console.error("[ai-chat-records] GET error:", error);
...
export async function POST(request: NextRequest) {
  ...
  if (messages.length === 0) {
    return NextResponse.json({ error: "저장할 메시지가 없습니다." }, { status: 400 });
  }
  ...
  const { data, error } = await supabase
    .from("ai_chat_records")
    .insert({ user_id: user.id, messages: sanitized })
    .select("id, created_at")
    .single();

  if (error) {
    console.error("[ai-chat-records] POST error:", error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  ...
  if (error) {
    console.error("[ai-chat-records] DELETE error:", error);
    return NextResponse.json(
